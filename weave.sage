...
class Interpreter:
  ...
  def weave_code(self):
    ...
    debug("Executing Sage snippet at line %d" % line_no_start)
  ...
  def weave_line(self, line):
    ...
    def evaluate(expr):
      debug("Evaluating Sage expression at line %d" % (self.line_no - 1))
...
def check_output_file(filename):
  ...
  with open(filename, "r") as in_file:
    for expectedLine in AUTOGENERATE_MSG:
      if in_file.readline().rstrip('\n') != expectedLine:
        error(('%s does not look like a sage-weave output file. '
               'Refusing to overwrite it.') % filename)
...
def main(args):
  ...
  info("Reading from %s" % ("stdin" if args.input == "-" else args.input))

  # Open output stream
  output_stream = sys.stdout if args.output == "-" else open(args.output, "wt")
  info("Writing to %s" % ("stdin" if args.output == "-" else args.output))
...
if __name__ == "__main__":
  ...
  info("This is sage-weave, version %s" % VERSION)
  try:
    main(args)
